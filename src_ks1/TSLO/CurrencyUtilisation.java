...
public class CurrencyUtilisation extends BSOrderXL
{
	...
	public void capturetotal() throws Exception
	{
		...
		String scrip1 = s.getRow(7).getCell(4).getStringCellValue();
	...
	public void PlaceAMOBuyOrder() throws Exception 
	  {
		...
		///report creation
		//report1 = new ExtentReports("D:\\Saleema\\TSLOcurrUtilisation.html");
		...
		////call login function
	  	...
	  	log = report1.startTest("Place TSLO Order Currency");
	  	
	  	///get multiple value from excel for utilization caln
	  	multiple = (int) s.getRow(3).getCell(4).getNumericCellValue();  	
		...
		///select scrip
		String xcelscrip = s.getRow(5).getCell(4).getStringCellValue();
		...
		///select buy/sell
		String value = s.getRow(6).getCell(4).getStringCellValue();
		w.findElement(By.xpath(".//input[@value='"+value+"']")).click();
		log.log(LogStatus.INFO, "Selected Value is " +value);
		
		////enter quantity
		int nooflots = (int) s.getRow(4).getCell(4).getNumericCellValue();  	
		...
		price = Double.parseDouble(pr1)-0.075;
		...
		if(tslochk.equals("YES"))
		{
			...
			///enter spread value
			int spread = (int) s.getRow(11).getCell(4).getNumericCellValue();
	...
	public void ManageCashAMO() throws Exception 
	  {		
		///report creation
		log = report1.startTest("Change TSLO Order Currency");
	  	...
	  	else
	  	{
			...
			if(status.equals("ACTIVE") || status.equals("OPN"))
			{
				...
				{		
					...
					nooflots = (int) s.getRow(4).getCell(9).getNumericCellValue();  	
					w.findElement(By.id("leg1_ord_qty")).sendKeys(String.valueOf(nooflots));
					log.log(LogStatus.INFO, "Modified No of lots is : " +nooflots);
					//wb.close();
					...
					String qt1 = w.findElement(By.xpath("//*[@id='head3_ord']")).getText();
	 ...
	 @Test(priority = 2)
	  ...
	  {
		 ///for report name
		  log = report1.startTest("Cancel TSLO Order Currency");
	...
	public void nouse()
	  {
		  //report1.endTest(log);
		  //report1.flush();
		  //w.get("D:\\Saleema\\TSLOcurrUtilisation.html");
	  }
}
