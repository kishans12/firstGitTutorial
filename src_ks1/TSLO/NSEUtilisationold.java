...
public class NSEUtilisationold extends BSOrderXL
{
	...
	public void PlaceAMOBuyOrder() throws Exception 
	  {
		...
		////call login function
	  	...
	  	log = report1.startTest("Place TSLO Order NSE");
		...
		///chk for TSLO
		String tslochk = s.getRow(10).getCell(1).getStringCellValue();
		...
		if(tslochk.equals("YES"))
		{
			...
			///enter spread value
			int spread = (int) s.getRow(11).getCell(1).getNumericCellValue();
	...
	public void ManageCashAMO() throws Exception 
	  {		
		///report creation
		log = report1.startTest("Change TSLO Order NSE");
	  	
		///to check for any error
		String amocheck = "Error";
	  	if(w.getPageSource().contains(amocheck))
	  	{										
	  		capturemsg = w.findElement(By.xpath("//*[@id=\'container\']/div/table/tbody/tr[2]/td/table/tbody/tr[2]/td")).getText();
	  	...
	  	else
	  	{
			...
			log.log(LogStatus.INFO, "Click on TSLO Order No " +orderno);
				...
				{	
					System.out.println("in elsse of modify ");
					FileInputStream f = new FileInputStream("D:\\Saleema\\KotakSecurities\\src\\uticash.xls");
					HSSFWorkbook wb = new HSSFWorkbook(f);
					HSSFSheet s = wb.getSheet("Utilisation");
					...
					quantity = (int) s.getRow(4).getCell(6).getNumericCellValue();  	
	 ...
	 @Test(priority = 2)
	  ...
	  {
		 ///for report name
		  log = report1.startTest("Cancel TSLO Order NSE");
	...
	public void nouse()
	  {
		  //report1.endTest(log);
		  //report1.flush();
		  //w.get("D:\\Saleema\\TSLONSEUtilisation.html");
	  }
