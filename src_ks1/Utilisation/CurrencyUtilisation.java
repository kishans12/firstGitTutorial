...
public class CurrencyUtilisation extends BSOrderXL
{
	...
	public void PlaceAMOBuyOrder() throws Exception 
	  {
		...
		///report creation
		//report1 = new ExtentReports("D:\\Saleema\\CurrencyUtilisation.html");
		...
		////call login function
	  	...
	  	log = report1.startTest("Place Cash Order Currency");
		...
		price = Double.parseDouble(pr1)-1;
		...
		///chk for AMO
		String amochk = s.getRow(8).getCell(4).getStringCellValue();
	...
	public void ManageCashAMO() throws Exception 
	  {		
		///report creation
		log = report1.startTest("Change Cash Order Currency");
	 ...
	 @Test(priority = 2)
	  ...
	  {
		 ///for report name
		  log = report1.startTest("Cancel Cash Order Currency");
		  ...
		  else
		  {
			...
			if(stckno !=null &&  !stckno.isEmpty())
			{
				...
				///check for error if cancelled/rejected
	...
	public void nouse()
	  {
		  report1.endTest(log);
		  report1.flush();
		  w.get("D:\\Saleema\\NSEUtilisation.html");
	  }
}
